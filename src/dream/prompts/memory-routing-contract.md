...
The highest-value evidence is explicit user feedback: corrections, intent, preferences, rationale, and criteria for a good result. Preserve the generalized lesson behind that feedback. A skill does not need to be a rigid workflow; it may teach judgment or package resources when they serve its triggered capability. One strong conversation, correction, code review, or resolved failure can justify a skill when the evidence is strong and the lesson generalizes; repeated evidence is not required.

For every proposed memory, identify the **experiential delta**: what became known through interaction or reflection that repository search alone would not reveal. Code paths and symbol names may appear only as minimal pointers needed to apply that lesson. Do not persist file maps, implementation tours, API inventories, or descriptions of what the current code does merely because they were discussed in a session.

#!/usr/bin/env bun
// Standalone visualizer for dream runs. NOT part of the dream SDK — it only
// reads the artifact directory a run leaves behind (per-batch
// input/output/trajectory/report, aggregate/*), so it can render any past run
// without the SDK, a backend, or credentials.
//
//   bun viz/dream-viz.ts <dream-id | latest | /path/to/run-root> [options]
//
//   --runs-root <dir>   where dream ids live (default ~/.letta/dream-runs,
//                       or $LETTA_DREAM_RUNS_ROOT)
//   --out <file>        output path (default <run-root>/viz.html)
//   --open              open the generated page in the default browser
//
// Output is one self-contained HTML file: every reflection agent's trajectory
// and edited memory clone, plus the aggregator's trajectory and the final
// memory filesystem, embedded as JSON with a vanilla-JS viewer.
...
// ---------------------------------------------------------------------------
// Run-root resolution: a dream id under the runs root, "latest", or a path.

function defaultRunsRoot(): string {
  return (
    process.env.LETTA_DREAM_RUNS_ROOT ?? join(homedir(), ".letta", "dream-runs")
  );
}

function resolveRunRoot(ref: string, runsRoot: string): string {
  ...
  if (!existsSync(runsRoot)) {
    throw new Error(`No dream runs recorded under ${runsRoot}`);
  }
...
/** normalized-v1 trajectory record (the only shape this tool reads). */
...
interface VizAgent {
  ...
  label: string;
  batchIndex?: number;
  /** Backend agent the pass ran on (the persistent reflector/aggregator). */
  agentId?: string;
...
function loadAggregatorAgent(runRoot: string): VizAgent | null {
  const aggregateDir = join(runRoot, "aggregate");
  ...
  // The aggregator edits the target memory filesystem in place. When the run
  // keeps its target inside the run root (memory/), snapshot that; otherwise
  // fall back to an aggregate/output snapshot if one was recorded.
  const memoryDir = existsSync(join(runRoot, "memory"))
    ? join(runRoot, "memory")
    : join(aggregateDir, "output");
  return {
    id: "aggregator",
    role: "aggregator",
    label: "aggregator",
...
function buildData(runRoot: string): VizData {
  ...
  const aggregator = loadAggregatorAgent(runRoot);
...
function main(): void {
  ...
  for (let i = 0; i < argv.length; i++) {
    const arg = argv[i];
    if (arg === "--runs-root") {
      runsRoot = argv[++i] ?? runsRoot;
    } else if (arg === "--out") {
      outPath = argv[++i];
    } else if (arg === "--open") {
      open = true;
    } else if (arg && !arg.startsWith("--") && !ref) {
      ref = arg;
    } else {
  ...
  if (!ref) {
    console.error(
      "usage: bun viz/dream-viz.ts <dream-id | latest | /path/to/run-root> [--runs-root <dir>] [--out <file>] [--open]",
    );
    process.exit(64);
  }

  const runRoot = resolveRunRoot(ref, runsRoot);
  if (!existsSync(runRoot)) {
    console.error(`dream run not found: ${runRoot}`);
    process.exit(66);
  }
  const data = buildData(runRoot);
  const html = renderHtml(data);
  const out = outPath ?? join(runRoot, "viz.html");
  writeFileSync(out, html, "utf8");
  console.log(`${data.runId}: ${data.agents.length} agent(s) → ${out}`);
  if (open) {
